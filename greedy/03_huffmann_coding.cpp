...
// make a tree storing data , left and right of any node
// first make a priority queue (basically min heap) to store all the given frequencies as node
// so top of pq will have smallest value
// pop twice to get two smallest values.........add them making newnode (also store left and right reference) and then push the sum again in pq
// repeat till only one node left in pq...it will be the root node
// now traverse 
